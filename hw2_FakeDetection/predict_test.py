...
'''
CLASSES = ('reliable', 'unreliable')


def decode_sequence(arr: np.array, reverse_word_index: dict):
...
def eval(predicts, labels):
    ...
    return np.sum(predicts == labels) / len(predicts)


def main(opt):
    model = load_model(opt['model'])
    with (opt['tokenizer']).open('rb') as f:
    ...
    predicts = np.round(outputs).astype(int)
